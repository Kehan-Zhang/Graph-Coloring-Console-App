//
//  normal_random_graph_generator.cpp
//  Algorithm Project
//
...
void normalRandomGraph(int n, int m) {
    clock_t start, end;
    start = clock();
    int edges = m;
    ...
    while(edges != 0) {
        double random1 = rand() * 1.0f / RAND_MAX;
        double random2 = rand() * 1.0f / RAND_MAX;
        int vertex1 = n / 2 + n / 5 * sqrt(-2 * log(random1)) * cos(2 * M_PI * random2);
        
        double random3 = rand() * 1.0f / RAND_MAX;
        double random4 = rand() * 1.0f / RAND_MAX;
        int vertex2 = n / 2 + n / 5 * sqrt(-2 * log(random3)) * cos(2 * M_PI * random4);
    ...
    writeFile("NormalRandomGraph", n, m, adjList);
    cout << "Generate Normal Random Graph Runtime:" << (double)(end - start) / CLOCKS_PER_SEC << endl;
